...
type Phase = "setup" | "loading" | "session" | "report";
...
export default function InterviewPage() {
  ...
  const { supported, listening, start, stop } = useSpeech(setAnswer);
  ...
  async function startInterview() {
    ...
    try {
      const res = await fetch("/api/questions", {
        ...
        body: JSON.stringify({ role, seniority, interviewType, jobDescription, count }),
      });
  ...
  async function finishInterview() {
    ...
    try {
      const res = await fetch("/api/report", {
        method: "POST",
        headers: { "content-type": "application/json" },
        body: JSON.stringify({ role, seniority, interviewType, turns }),
      });
      ...
      <div className="min-w-0">
        {demoMode && phase !== "setup" && (
          <div className="mb-5 rounded-xl border border-amber-400/20 bg-amber-400/10 px-4 py-2.5 text-xs text-amber-200">
            Demo mode — running on the built-in question bank. Add an{" "}
            <code className="font-mono">ANTHROPIC_API_KEY</code> for live AI questions and feedback.
          </div>
        )}
        {error && (
        ...
        {phase === "setup" && (
          <SetupView
            {...{
              role, setRole, seniority, setSeniority, interviewType,
              setInterviewType, jobDescription, setJobDescription, count, setCount, startInterview,
            }}
          />
        )}
        ...
        {phase === "session" && questions.length > 0 && (
          <SessionView
            ...
            speech={{ supported, listening, start, stop }}
  ...
  return (
    <div className="glass animate-fade-up p-6 sm:p-8">
      ...
      <div className="mt-7 space-y-6">
        ...
        <div>
          ...
          <div className="flex flex-wrap gap-2.5">
            {SeniorityLevels.map((s) => (
              <button
                key={s}
                onClick={() => p.setSeniority(s)}
                className={`chip ${p.seniority === s ? "chip-on" : "chip-off"}`}
              >
                {SENIORITY_LABEL[s]}
              </button>
            ))}
        ...
        <button onClick={p.startInterview} disabled={!p.role.trim()} className="btn-primary w-full">
          Start interview →
        </button>
      </div>
    </div>
  ...
  return (
    <div className="glass grid min-h-[320px] place-items-center p-10 text-center">
      <div>
        ...
        <p className="mt-1 text-xs text-slate-500">The coach is preparing your interview.</p>
      </div>
    </div>
  );
}
  ...
  return (
    <div className="animate-fade-up space-y-5">
      ...
      <div className="glass p-6 sm:p-7">
        ...
        {p.question.rationale && (
          <p className="mt-3 text-xs text-slate-500">💡 {p.question.rationale}</p>
        )}
      </div>
            ...
            {(["situation", "task", "action", "result"] as const).map((k) => (
              <span
                ...
                className={`rounded-lg px-2.5 py-1 text-xs font-medium capitalize ${
                  star[k] ? "bg-accent-500/20 text-accent-400" : "bg-white/5 text-slate-500"
                }`}
              >
                {star[k] ? "✓" : "○"} {k}
              </span>
        ...
        <div>
          ...
          <ul className="mt-2 space-y-1.5 text-sm text-slate-300">
            {feedback.improvements.map((s, i) => (
              <li key={i} className="flex gap-2"><span className="text-amber-300">→</span>{s}</li>
            ))}
          </ul>
        </div>
      </div>
  ...
  return (
    <div className="animate-fade-up space-y-5">
        ...
        <ul className="mt-4 space-y-2.5">
          {report.actionItems.map((a, i) => (
            <li key={i} className="flex gap-3 text-sm text-slate-300">
              <span className="grid h-5 w-5 flex-shrink-0 place-items-center rounded-md bg-brand-500/20 text-xs font-bold text-brand-300">
                {i + 1}
              </span>
              {a}
            </li>
      ...
      <div className="flex flex-col gap-3 sm:flex-row">
        ...
        <a href="/" className="btn-ghost flex-1 text-center">Back home</a>
      </div>
    </div>
  );
}
                ...
                <li key={h.id} className="flex items-center justify-between rounded-lg bg-white/[0.03] px-3 py-2 text-xs">
                  <div className="min-w-0">
                    ...
                    <div className="text-slate-500">
                      {TYPE_META[h.interviewType].label} · {new Date(h.date).toLocaleDateString()}
                    </div>
                  </div>
                  <span className={`ml-2 font-bold tabular-nums ${h.overallScore >= 75 ? "text-accent-400" : h.overallScore >= 55 ? "text-brand-300" : "text-amber-300"}`}>
                    {h.overallScore}
                  </span>
                </li>
              ))}
            </ul>
            <button onClick={onClear} className="mt-4 text-xs text-slate-500 underline-offset-2 hover:text-slate-300 hover:underline">
              Clear history
            </button>
          </>
        )}
